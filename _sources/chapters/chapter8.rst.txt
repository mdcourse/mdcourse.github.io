...
- 2) Two particles are chosen randomly, and their respecitive positions are exchanced.
- 3) The energy of the system after the move, :math:`E_\text{pot}^\text{trial}`, is measured.
- 4) Just like with Monte Carlo move, we then have to decide to keep or reject
  the move. This is done by calculating the difference in energy between the
  trial and the initial configurations:
...
In general, both Monte Carlo swap moves and displacements are being performed
